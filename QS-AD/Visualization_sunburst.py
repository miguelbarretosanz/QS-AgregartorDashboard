...
def get_dataset (src, unique_days_list, selected_day, df_activity_colors):
    """
    Select the vectors with the same selected day.
    Creates the dataframe to be ploted by make_plot()
...
#I tried to use if __name__ == '__main__':  but it is not possible with "bokeh server" as 
# is described on https://groups.google.com/a/continuum.io/forum/#!topic/bokeh/G9Sy7XFGQWQ

LC_data = pd.read_csv('../data/Life Cycle/example/LC_export_22-4-2018.csv')
...
#create a new dataset where the events that still happening after midnigth are
#assingned to the next day as a new event        
